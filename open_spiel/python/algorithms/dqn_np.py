...
class ReplayBuffer(object):
  ...
  def __init__(self, replay_buffer_capacity, num_actions, state_representation_size):
    # self._replay_buffer_capacity = replay_buffer_capacity
    # self._data = []
    # self._dataNum = 0
    # self._next_entry_index = 0

    self._actions = np.zeros((replay_buffer_capacity,), dtype=np.int32)
    self._rewards = np.zeros((replay_buffer_capacity,), dtype=np.float32)
    self._is_final_step = np.zeros((replay_buffer_capacity, ), dtype=np.float32)
    self._info_states = np.zeros((replay_buffer_capacity, state_representation_size), dtype=np.float32)
    self._next_info_states = np.zeros((replay_buffer_capacity, state_representation_size), dtype=np.float32)
    self._legal_actions_masks = np.zeros((replay_buffer_capacity, num_actions), dtype=np.float32)

    self.max_size = replay_buffer_capacity
    self.ptr = 0
    self.size = 0
  ...
  # def add(self, element):
    ...
    # if self._dataNum < self._replay_buffer_capacity:
  ...
  def add(self, prev_action, reward, is_final_step, info_state, next_info_state, legal_actions):
    self._actions[self.ptr] = prev_action
    self._rewards[self.ptr] = reward
    self._is_final_step[self.ptr] = is_final_step
    self._info_states[self.ptr][:] = info_state
    self._next_info_states[self.ptr][:] = next_info_state
    self._legal_actions_masks[self.ptr].fill(0.0)
    self._legal_actions_masks[self.ptr][legal_actions] = 1.0
  ...
  def sample(self, num_samples):
    ...
    idxs = np.random.choice(self.size, size=num_samples, replace=False)
    return dict(actions=self._actions[idxs],
                rewards=self._rewards[idxs],
                is_final_step=self._is_final_step[idxs],
                info_states=self._info_states[idxs],
                next_info_states=self._next_info_states[idxs],
                legal_actions_mask=self._legal_actions_masks[idxs],)
...
class DQN(rl_agent.AbstractAgent):
  ...
  def __init__(self,
    ...
    self._replay_buffer = replay_buffer_class(replay_buffer_capacity, num_actions, state_representation_size)
  ...
  def add_transition(self, prev_time_step, prev_action, time_step):
    ...
    # legal_actions_mask = np.zeros(self._num_actions, dtype=np.float16)
    ...
    self._replay_buffer.add(prev_action,
  ...
  def learn(self):
    ...
    is_final_step = transitions["is_final_step"]
    info_states = transitions["info_states"]
    next_info_states = transitions["next_info_states"]
    legal_actions_mask = transitions["legal_actions_mask"]
